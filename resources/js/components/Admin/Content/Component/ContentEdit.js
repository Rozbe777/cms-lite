...
const ContentEdit = ({actionResult, contentDataUpdate}) => {
    ...
    const onSubmit = () => {
        ...
        contentFormResult.append("id", contentForm.id)
        contentFormResult.append("_token", TOKEN)
        ...
        if (contentDataClone.title && contentDataClone.title !== '') {
            ...
            swalAccept("ویرایش محتوا").then(resSwal => {
                if (resSwal.value) {
                    api.setContentDataUpdate(contentFormResult);
                    api.update().then(res => {
