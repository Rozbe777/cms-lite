...
const InputMini = ({newData , code,slices ,  label, placeHolder,  required}) => {
    ...
    return(
        ...
        <fieldset className="form-group" style={{margin: 7}} onClick={e => HandleClick(e, code)}>
            ...
            <label style={{color: formTheme.textColor}} htmlFor={random + "_label"}>{initialFormDataMiniText[code].Mandatory ? initialFormDataMiniText[code].title ? initialFormDataMiniText[code].title + "(*)" : 'عنوان (*)' : initialFormDataMiniText[code].title ? initialFormDataMiniText[code].title : 'عنوان' }</label>
            ...
            <div className={"row"} style={{padding: 0, margin: 0}}>
                <div className={"col-8"} style={{padding: 0, margin: 0}}>
                    <p style={{color: formTheme.textColor + " !important"}}>
                        <small
                            style={{color: formTheme.textColor}}>{initialFormDataMiniText[code].description ? initialFormDataMiniText[code].description : 'توضیحات مختصر فیلد'}</small>
                    </p>
                </div>
                <div className={"col-4"} style={{padding: 0, margin: 0, color: formTheme.textColor + " !important"}}>
                    <p style={{textAlign: 'left', fontSize: '11px'}}>
                        <small
                            style={{color: formTheme.textColor}}>{initialFormDataMiniText[code].maximum ? counter < initialFormDataMiniText[code].maximum && counter > 0 ? "کاراکتر (" + counter + ")" : initialFormDataMiniText[code].maximum == counterUp ? "غیرمجاز" : "کاراکتر (" + initialFormDataMiniText[code].maximum + ")" : 'تعداد کارکتر'}</small>
                    </p>
                </div>
            </div>
