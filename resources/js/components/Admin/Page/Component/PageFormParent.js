...
export const PageFormParent = ({
                                       ...
                                       pageData,
                                       editorData,
                                       allPage, // this data using to display multioption parernt id
                                       fileChange,
                                       handleMetaData,
                                       pageParentId,
                                       edit
    ...
    let namePage = $("input#title").val() + "";

    const [editInComponent, setEditInComponent] = useState(false)
    const dataUpdateParse = pageData ? pageData : '';
    ...
    const handleSwither = (e, state, name) => {
        switch (name) {
            ...
            case 'showMenu' :
                ...
                return true;
                case 'is_index' :
                componentHandler.handleSwitchIsIndex(e, state, changeCheck);
                return true;
            default :
                return true;
        }
    ...
    console.log("_____" , dataUpdateParse)

    function handleMakeName() {
        let pageName = dataUpdateParse.title;
        const min = 1;
        const max = 1000;
        const rand = Number(min + Math.random() * (max - min)).toFixed(0);
        return pageName + rand + "_کپی";
    }
    ...
    function _renderSlug() {
        ...
        if (slugManage) {
            let slugText;
            if (actionType === "duplicate"){
                slugText = helperFunction.contentFormData(edit ? tit : handleMakeName(pageData.name));
            }else if (actionType === "edit"){
                slugText = helperFunction.contentFormData(edit ? tit : pageData.title);
            }else{
                        ...
                        <div className={"row"} style={{padding: '25px'}}>
                            ...
                            <div className={"col-12"}>
                                ...
                                <BigSwitcher status={states => componentHandler.HandlerBigSwitcher(states, changeCheck)}
                                             ...
                                             defaultStatus={metaDataUpdate.robots ? metaDataUpdate.robots : 'false'}
                                             default={''}
