...
const ProductAdd = ({types, result}) => {

    const [allFiles, setAllFiles] = useState([]);
    const [metaData, setMetaData] = useState({robots: false});
    ...
    let normalDefalutAttr = NoralizeFetures(defaultCol, types, counter.num);

    const [contentNew, setContentNew] = useState('');
    const [chipset, setChipset] = useState([]);
    const [chipsetTags, setChipsetTags] = useState([]);

    const [priceData, setPriceData] = useState(defaultCol);
    ...
    const onSubmit = (e) => {
        ...
        if (checkValueFetures) {
            ...
            if (formDataClone.title && formDataClone.title !== '') {
                ...
                swalAccept("افزودن محصول جدید").then(resSwal => {
                    if (resSwal.value) {
                        productApi.create(productFormData).then(res => {
        ...
        <>
            <div id={"category_add_pop_base"}>
                ...
                <div className={"col-12 bottom-footer"}>
                    <Footer actionType={"save"} editStatus={edit} onCancel={onClose} onClicked={onSubmit}/>
                </div>
            </div>
